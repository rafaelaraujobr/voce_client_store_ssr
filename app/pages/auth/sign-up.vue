            ...
            <q-card flat>
              ...
              <q-card-section class="q-pt-none q-gutter-y-sm">
                <q-btn
                  ...
                  icon-right="sym_r_chevron_right"
                  :disabled="
                    !checkPasswordMatch(form.password, form.confirmPassword) ||
                    !dataConsent
                  "
                  type="submit"
                  dense
...
const refDocument = ref<any>(null);
const errorDocument = ref<boolean>(false);
const errorName = ref<boolean>(false);
const erroMessageName = ref<string>("");
const errorDate = ref<boolean>(false);
const erroMessageDocument = ref<string>("");
...
const onCreateAccount = async () => {
  ...
  try {
    const {status } = await createAccountService(account);
    if (status === 1) {
      step.value++;
    } else {
      refDocument.value.focus();
      errorDocument.value = true;
      erroMessageDocument.value = "Este documento invalido.";
    }
  } catch (error: any) {
    console.log(error);
  } finally {
