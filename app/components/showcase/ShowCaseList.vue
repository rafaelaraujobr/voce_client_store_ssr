<template>
  <q-infinite-scroll ref="scrollTargetRef" :offset="800" debounce="500">
          ...
          <q-card-section class="row q-pa-none q-ma-none justify-center items-center">
            <div class="col-4 col-md-12 bg-default q-py-sm">
              <q-img :src="product.image" fit="contain" spinner-color="primary" class="product-image" height="118px"
                ...
                <template #error>
                  <div class="absolute-full flex flex-center bg-secondary text-white text-center"
            ...
            <q-btn v-if="!productsInCart.find((p) => p.id === product.id)" color="primary" icon="mdi-cart-plus"
            ...
            <q-btn v-else color="negative" icon="mdi-cart-remove"
              :class="$q.screen.lt.md ? 'absolute-bottom-right' : 'absolute-top-right'" flat dense :style="{
                bottom: $q.screen.lt.md ? '8px' : 'auto',
                top: $q.screen.lt.md ? 'auto' : '8px',
                right: '8px',
    ...
    <div class="row justify-center items-center q-mt-md q-py-md">
      <q-btn v-if="!loadingProducts && products.length < totalProducts" :loading="loadingProducts" color="primary"
        label="Carregar mais produtos" class="q-mt-md" padding="sm md" @click="loadMoreProducts(slug as string)" />
    </div>
