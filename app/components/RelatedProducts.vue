  ...
  <q-card v-if="relatedProducts && relatedProducts.length > 0" flat class="related-products">
    ...
    <q-card-section class="q-pa-none">
      <q-tab-panels v-model="currentGroup" animated>
        <q-tab-panel v-for="(group, index) in productRelatedGroup" :key="index" :name="index">
          <div class="row q-col-gutter-md">
            <div v-for="product in group" :key="product.id" class="col-xs-12 col-sm-6 col-md-4">
              <q-card
                flat
                bordered
                  ...
                  <div class="col-8 col-md-12 q-py-md">
                    <q-item>
                      <q-item-section>
                        ...
                        ">
                          <q-item-label lines="2" class="text-subtitle1 text-negative"
      ...
      <div class="row justify-end q-gutter-x-sm q-mt-md">
        <q-btn icon="mdi-chevron-left" dense padding="sm" text-color="primary" class="bg-default"
          :disable="currentGroup === 0" @click="prevProductGroup" />
        <q-btn icon="mdi-chevron-right" class="bg-default" dense padding="sm" text-color="primary"
          :disable="currentGroup === productRelatedGroup.length - 1" @click="nextProductGroup" />
      </div>
    </q-card-section>
  </q-card>
</template>
...
const productRelatedGroup = computed(() => {
  ...
  const groups = [];
  // Agrupa em 3 produtos por grupo (responsivo: 1 em xs, 2 em sm, 3 em md+)
